...
INSERT INTO `orderItems` VALUES (1,10,1,00000000020,NULL,NULL),(2,11,1,00000000014,NULL,NULL),(4,11,2,00000000076,'2018-08-24 03:15:21','2018-08-24 03:15:21'),(5,10,2,00000000037,'2018-08-24 03:15:21','2018-08-24 03:15:21');
...
INSERT INTO `Orders` VALUES (1,1,1,NULL,NULL,'0'),(2,1,1,'2018-08-24 03:15:21','2018-08-24 03:36:26','2');
...
/*!50001 VIEW `view_order` AS select `a`.`id` AS `idOrder`,`d`.`username` AS `username`,sum((`c`.`price` * `b`.`qty`)) AS `total` from (((`orders` `a` left join `orderitems` `b` on((`a`.`id` = `b`.`idorder`))) left join `products` `c` on((`c`.`id` = `b`.`idproduct`))) left join `users` `d` on((`d`.`id` = `a`.`iduser`))) group by `a`.`id`,`d`.`username` */;
...
-- Dump completed on 2018-08-24 10:47:11
